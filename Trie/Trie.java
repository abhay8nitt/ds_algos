...
public class Trie {
    ...
    public void deleteRecursive(String word) {
        _deleteRecursive(root, word, 0);
    }

    private void _deleteRecursive(TrieNode root, String word, int idx) {

    }

    public void deleteFromTrie(String param) {
        TrieNode cur = root;
        int size = param.length();
        if (param.length() == 1) {
            Character c1 = param.charAt(0);
            Map<Character, TrieNode> temp = cur.getNext();
            if (temp.containsKey(c1)) {
                if (temp.size() == 1) {
                    temp.get(c1).setNext(new HashMap<>());
                } else {
                    if (temp.get(c1).isWord) {
                        temp.get(c1).isWord = false;
                    } else {
                        System.out.println("\n String " + param
                                + " is not found in trie");
                    }
                }
            } else {
                System.out.println("\n String " + param
        ...
        TrieNode prev = cur;
        TrieNode deletable = null;
        TrieNode deleteParent = null;
        for (int i = 0; i < size; i++) {
            Character c1 = param.charAt(i);
            Map<Character, TrieNode> temp = cur.getNext();
            if (temp.containsKey(c1)) {
                if (temp.size() == 1) {
                    if (deletable == null || cur.isWord()) {
                        deletable = temp.get(c1);
                        deleteParent = cur;
                    }
                    prev = cur;
                } else {
        ...
        if (!cur.isWord()) {
            System.out.println("\n String " + param + " is not found in trie");
            return;
        }
        if (cur.getNext().size() != 0) {
            cur.isWord = false;
            return;
        }
        if (deletable != null) {
            deleteParent.setNext(new HashMap<>());
        } else {
            Map<Character, TrieNode> temp = prev.getNext();
            temp.remove(cur.name);
        }
    }
}